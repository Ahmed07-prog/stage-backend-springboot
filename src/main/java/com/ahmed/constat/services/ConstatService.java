...
public class ConstatService {
   ...
   /* public void deleteConstat(Long userId) {
        constatRepository.deleteConstatByUserIdWithDetails(userRepository.findById(userId).get().getId());
    }
    public Constat findConstat(Long userId) throws Exception {
        return constatRepository.findByIdWithDetails(userRepository.findById(userId).get().getId());}
*/
}
